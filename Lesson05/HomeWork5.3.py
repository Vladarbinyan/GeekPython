...
while True:
    filename = input(
        'Введите имя файла в текущей директории, для выхода без чтения оставьте поле пустым (образец zp)>>> ')
...
while True:
    filename = input(
        'Введите имя файла в текущей директории, для выхода без чтения оставьте поле пустым (образец words) >>> ')
    if is_empty(filename):
        print('Чтение отменено, выход')
        break
    elif not path.exists(filename):
        ...
        if file_write('ru_' + filename, in_string):
            print(f'Результат записан в файл: ru_{filename}')
        break

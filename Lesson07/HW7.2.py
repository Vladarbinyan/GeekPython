...
class Clothing(ABC):
    ...
    def textile_calc(self):
        pass  # Заключаем контракт на реализацию расчета расхода ткани.

    @property
    @abstractmethod
    def name(self):
        pass  # Продемонстрирую работу с декоратором @property
...
class Suit(Clothing):
    ...
    @property
    def name(self):
        return f'Костюм {self.__name}'


suit01 = Suit(175.2, 'Франт')
coat01 = Coat(50.0, 'Аристократ')

print(suit01.name)
print(f'Для пошива {suit01.name}, потребуется {coat01.textile_calc()} условных единиц ткани')